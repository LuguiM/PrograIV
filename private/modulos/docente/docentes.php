...
class Docente{
    ...
    private function administrar_docente(){
        ...
        if( $this->respuesta['msg']=='ok'){
            if($accion=='nuevo'){
                $this->db->consultas('
                INSERT INTO docentes(idDocente,codigo,nombre,dirrecion,municipio,departamento,telefono,fechaNacimiento,sexo) VALUES(?,?,?,?,?,?,?,?,?)','ss',
                $this->datos['idDocente'],$this->datos['codigo'], $this->datos['nombre'],$this->datos['direccion'],
                $this->datos['municipio'],$this->datos['departamento'],$this->datos['telefono'],$this->datos['fechaNacimiento'],$this->datos['sexo']
            );
            return $this->db->obtener_respuesta();
            }else if ($accion=='modificar'){
                $this->db->consultas('
                UPDATE docentes SET INTO codigo=?,nombre=?,dirrecion=?,municipio=?,departamento=?,telefono=?,fechaNacimiento=?,sexo=? WHERE idDocente=?',
